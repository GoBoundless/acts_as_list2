        ...
        private
          ...
          def check_scope
            if changes.include?("#{scope_name}")
              ...
              if changes.include?("#{position_column}")
                old_position = changes["#{position_column}"].first
                new_position = changes["#{position_column}"].last
                self["#{position_column}"] = old_position
              end
              self["#{scope_name}"] = old_scope_id
              send("decrement_positions_on_lower_items")
              self["#{position_column}"] = new_position if !new_position.nil?
